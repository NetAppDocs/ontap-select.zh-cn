...
集群扩展:: ONTAP Select 9.13.1引入了集群扩展功能、可提供高可用性(HA)保护、防止节点中断对集群提供数据的能力造成影响。从ONTAP Select 9.13.1开始、您可以通过将单节点集群配置为HA对来将这两个节点扩展为双节点集群。操作步骤会自动执行、并可通过ONTAP Select Deploy使用命令行界面或Web界面启动。要执行此操作步骤、您必须满足特定的硬件和软件要求。 link:concept_cluster_expansion.html["了解更多信息。"]。
