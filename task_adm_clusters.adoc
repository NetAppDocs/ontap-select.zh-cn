...
. 选择集群右侧的 image:icon_kebab.gif["选项"] 并选择 *Take Offline*。
+
如果脱机选项不可用，则表示集群已处于脱机状态。

. 在弹出窗口中选择 *是* 确认请求。
. 偶尔选择 *刷新* 以确认集群处于脱机状态。
. 要使集群恢复联机，请选择 image:icon_kebab.gif["选项"] 并选择 *Bring Online*。
. 偶尔选择 *刷新* 以确认集群处于联机状态。
...
. 使用管理员帐户登录到 Deploy 实用程序 Web 用户界面。
. 选择页面顶部的*集群*选项卡，然后从列表中选择所需的集群。
. 选择集群右侧的 image:icon_kebab.gif["选项"] 并选择 *Delete*。
+
如果删除选项不可用，则表示集群未处于脱机状态。

. 偶尔选择 *刷新* 以确认集群已从列表中删除。
...
.步骤
. 使用管理员帐户登录到 Deploy 实用程序 Web 用户界面。
. 选择页面左上角的 *Clusters* 选项卡，然后从列表中选择所需的集群。
. 选择页面右侧的 image:icon_kebab.gif["选项"] 并选择 *Cluster Refresh*。
