...
NetApp建议使用 ESX 8.0 或更高版本，并使用 NVMe 磁盘作为托管系统磁盘的数据存储。此配置可为NVRAM分区提供最佳性能。


NOTE: 在 ESX 8.0 或更高版本上安装时，无论系统磁盘位于 SSD 还是 NVMe 磁盘上， ONTAP Select都会使用 vNVMEe 驱动程序。这会将虚拟机硬件级别设置为 13，与 ESX 8.0 及更高版本兼容。
