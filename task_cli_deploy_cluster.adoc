...
在生产环境中部署 ONTAP Select 集群之前，您必须购买存储容量许可证并下载关联的许可证文件。您可以使用_Capacity Tier_模式为每个节点上的存储授予许可、也可以使用_Capacity Pools_模式为共享池授予许可。
...
.开始之前
您必须具有节点的配置信息。应在Deploy实用程序中上传并安装容量层许可证文件。
