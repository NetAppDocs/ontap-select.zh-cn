...
== 将存储连接到 ONTAP Select 节点

您必须配置 ONTAP Select 集群中每个节点使用的存储。必须至少为每个节点分配一个存储池。

您必须已使用 VMware vSphere 创建存储池。

此操作步骤 假定使用本地硬件 RAID 控制器。您应在 ONTAP Select 集群中的每个节点上配置存储。
