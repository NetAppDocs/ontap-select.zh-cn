...
下图显示了这些网络，该图展示了在 VMware vSphere 平台上运行的四节点ONTAP Select集群。六节点、八节点、十节点和十二节点集群具有类似的网络布局。
...
有关详细信息，请参阅 link:reference_plan_best_practices.html#two-node-stretched-ha-metrocluster-sds-best-practices["双节点延伸型 HA （ MetroCluster SDS ）最佳实践"] 一节。
...
NOTE: 端口 e0e 由 ONTAP 保留用于内部复制流量。因此，端口和托管 LIF 在 ONTAP CLI 或 System Manager 中都不可见。此接口配置为使用自动生成的链路本地 IP 地址，您无法分配备用 IP 地址。此网络端口需要使用巨型帧（7500 到 9000 MTU）。
