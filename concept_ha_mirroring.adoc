...
具有中介的双节点群集可以跨越两个数据中心。有关详细信息，请参阅部分link:reference_plan_best_practices.html#two-node-stretched-ha-metrocluster-sds-best-practices["双节点延伸型 HA （ MetroCluster SDS ）最佳实践"]。



== 镜像聚合

ONTAP Select集群由 2 到 12 个节点组成。每个 HA 对包含两个用户数据副本，通过 IP 网络跨节点同步镜像。这种镜像对用户是透明的，它是数据聚合的一个属性，在数据聚合创建过程中会自动配置。
