...
.ESXi 主机服务器硬件要求
您的 ESXi 虚拟机监控程序主机必须满足几个最低资源要求。请检查部署 ONTAP Select 的主机是否满足以下基本要求：
...
.网络连接
检查 ONTAP Select Deploy 虚拟机网络接口是否已配置并具有单个管理 IP 地址。您可以使用 DHCP 动态分配 IP 地址或手动配置静态 IP 地址。

根据您的部署决策， Deploy 虚拟机必须能够连接到 vCenter 服务器， ESXi 虚拟机管理程序主机及其管理的 ONTAP Select 节点。您必须将防火墙配置为允许所需的流量。

Deploy 使用 VMware VIX API 与 vCenter 服务器和 ESXi 主机进行通信。最初，它在 TCP 端口 443 上使用 SOAP over SSL 建立连接。此后，使用端口 902 上的 SSL 打开连接。此外，Deploy 还会发出 PING 命令，以验证您指定的 IP 地址上是否存在 ESXi 主机。

此外，Deploy 必须能够使用以下协议与 ONTAP Select 节点和集群管理 IP 地址进行通信：
