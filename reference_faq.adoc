...
* 对于使用软件 RAID 的 ONTAP Select (KVM) 和 ONTAP Select (ESX)，无法恢复到不支持软件 RAID 的先前版本。此外，在 ESXi 上新安装的 ONTAP Select 9.5 或更高版本使用 VMXNET3 网络驱动程序，并在可能的情况下使用 vNMVE 驱动程序。这些新安装无法还原到早期版本的 ONTAP Select。
