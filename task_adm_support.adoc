...
. 选择页面顶部的*管理*选项卡。
. 选择 *Settings & AutoSupport*，然后选择 image:icon_pencil.gif["编辑"]。
. 提供适合您环境的配置数据，然后选择 *修改*。
+
...
. 使用管理员帐户登录到 Deploy 实用程序 Web 用户界面。
. 选择页面顶部的*管理*选项卡。
. 选择 *Events & Jobs*，然后选择 *Events*。
. （可选）选择 *Filter* 并创建一个过滤器以限制显示的事件消息。
...
. 使用管理员帐户登录到 Deploy 实用程序 Web 用户界面。
. 选择页面顶部的*管理*选项卡。
. 选择 *Settings & AutoSupport* ，然后选择 image:icon_kebab.gif["选项"]。
...
. 或者，在 *AutoSupport 历史记录* 下选择正确的包，然后选择下载图标将 AutoSupport 文件保存到本地工作站。
