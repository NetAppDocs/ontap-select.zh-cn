...
在ONTAP Select集群中，内部流量和外部流量使用称为端口组的虚拟二层网络对象进行隔离。正确分配这些端口组的 vSwitch 非常重要，尤其是对于负责提供集群、HA 互连和镜像复制服务的内部网络而言。这些网络端口的网络带宽不足会导致性能下降，甚至影响集群节点的稳定性。因此，4、6、8、10 和 12 节点集群要求内部ONTAP Select网络使用 10Gb 连接；不支持 1Gb 网卡。不过，可以对外部网络做出权衡，因为限制传入ONTAP Select集群的数据流量不会影响其可靠运行的能力。
...
NOTE: 与内部流量类似，外部流量可以在 vSwitch 层（VST）和外部交换层（EST）进行标记。此外，ONTAP Select VM 本身还可以在称为 VGT 的过程中对外部流量进行标记。有关更多详细信息，请参阅 link:concept_nw_data_mgmt_separation.html["数据和管理流量隔离"] 部分。
